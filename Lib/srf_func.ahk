...
If !(srf_func_table[modeMatch.Value[1], "Name"]="Cliphistory"&&A_ThisHotkey~="Numpad"){
    ...
    ; {
    ;     DrawHXGUI(ToolTipText, srf_for_select_obj, Caret.X, Caret.Y+Caret.H+Caret.H, srf_direction, srf_all_Input~="^" (func_key="\"?"\\":func_key) "[a-z]+$"?SymbolFont:TextFont)
    ; } Else
...
GetCaretPos(Byacc:=1){
    ...
    If (A_CaretX=""){
        ...
        If (Caretx=0&&Carety=0){
            ...
            Return {x:Caretx+6,y:Carety,h:Max(Careth,0),t:"Acc",Hwnd:Hwnd} ; 这里故意在 x 坐标上增加了 5 个像素点，是为了照顾 VSCode vim 的中文输入法问题
        }
    } Else
    Return {x:A_CaretX+6,y:A_CaretY,h:30,t:"Caret",Hwnd:Hwnd} ; 这里也在 x 轴上增加了 5 个像素点，理由同上
}
...
class CloudinputApi
{
    ...
    get(str){
        ...
        If (oldOS){
            If (this.req="")
                this.req:=ComObjCreate("WinHttp.WinHttpRequest.5.1"), cb:=ObjBindMethod(this, "BDcallback")
            SetTimer % cb, Off
            try If (str){
