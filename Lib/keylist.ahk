...
ToolTipInputStatus(){
    ...
    If (!srf_inputing){
        ...
        Else {
            ...
            ; 这里会在光标出显示土司条，提示当前切换到了中文或者英文，其实我认为是不太需要的，因为托盘里面可以看到实时的状态
            ; ToolTip, % fg, Caret_.X+5, Caret_.Y+30
            ; ToolTip, % fg, 10, 10
            ; ToolTip, % Caret_.X, 10, 10
            ; ToolTip, % Caret_.Y, 100, 30
            ; 显示当前捕捉到的光标的坐标和高度信息
            ; fanystr := "`n"
            ; fanymsg := Caret_.X fanystr Caret_.Y fanystr Caret_.H
            ; ToolTip, % fanymsg, 10, 50
            SetYzLogo(fg="中", 0) 
