...
Option_Adv:
	...
	Gui, 3:Add, CheckBox, x+25 yp gini_Settings vAutoupdatefg Checked%Autoupdatefg%, 自动更新
	Gui, 3:Add, CheckBox, x+25 yp gEnableUIAccess vUIAccess Checked%UIAccess%, UIAccess
	Gui, 3:Add, CheckBox, x+25 yp gini_Settings vMemoryDB Checked%MemoryDB%, 内存数据库
	Gui, 3:Add, CheckBox, % "xs y+10 gInstallTSF Checked" (DllCall(A_ScriptDir "\tsf\Yzime" (A_PtrSize=8?"64":"") ".dll\IsYzimeInstall")=1), 启用TSF
	...
	Gui, 3:Add, Text, % "xm+35 yp+25 w" tvar-30, 影子输入法是由河许人发起，天黑请闭眼深度开发的简单、简洁、高度自定义输入法。影子输入法起源于AutoHotkey高手群 Hello_srf开发的柚子输入法，融合了承影和jip输入法的理念。 影子输入法为开源、绿色、安全的输入法。请放心使用!
	GuiControlGet, tempP, 3:Pos, Static29
	GuiControl, 3:Move, 简介, % "h" tempPh+40
	Gui, 3:Add, Text, xm+10 y+25 w80 Right, 源代码:
	Gui, 3:Add, Link, x+10 yp, <a href="https://gitee.com/orz707/Yzime">https://gitee.com/orz707/Yzime</a>
	Gui, 3:Add, Picture, xm+15 yp w20 h-1 Icon11, %DataPath%Yzime.icl
	Gui, 3:Add, Text, xm+10 y+5 w80 Right, 帮助:
	Gui, 3:Add, Link, x+10 yp, <a href="https://gitee.com/orz707/Yzime/wikis/`%E5`%BF`%AB`%E9`%80`%9F`%E5`%BC`%80`%E5`%A7`%8B?sort_id=1844118">https://gitee.com/orz707/Yzime/wikis</a>
	Gui, 3:Add, Picture, xm+15 y+15 w20 h-1 Icon10, %DataPath%Yzime.icl
	Gui, 3:Font, s10 Bold, %GUIFont%
	...
	Gui, 3:Font, s10 norm, %GUIFont%
	Gui, 3:Add, Text, % "y+5 w" tvar-60, 非常感谢大家的支持, 感谢Hello_srf、胡杨、心如止水、jeff、大唐（云输入）、is1or0（大量优秀建议）、赵小峰（词库）、琪C（图标）、李子（词库管理源码参考）
...
switchtab:
	...
	GuiControlGet, tempP, 3:Pos, % (tvar=3?"候选窗口":tvar=2?"输入法热键":tvar=4?"命令管理":tvar=1?"其他选项":"Static40")
	GuiControl, 3:Move, SysTabControl321, % "h" tempPy+tempPh
	Gui, 3:Show, AutoSize
Return
...
Starting:
	...
	If Startingup {
		If A_IsCompiled
			tvar = schtasks /Create /TN "影子输入法" /TR "'%A_ScriptFullPath%'" /SC ONLOGON /DELAY 0000:05
		Else
			tvar := "schtasks /Create /TN ""影子输入法"" /TR ""'" StrReplace(A_ScriptFullPath, ".ahk", ".exe") "' '" A_ScriptFullPath "'"" /SC ONLOGON /DELAY 0000:05"
...
WM_MOUSEMOVE(wParam, lParam, Msg, Hwnd){
	...
	static buf, lasthwnd:=0, lastpos:=0
		, tips:={Startingup:"此功能需要系统的任务计划服务支持",Autoupdatefg:"勾选自动更新后会检测最新版的exe文件并提示更新，并不会更新源码部分，`n源码请在Gitee、百度网盘、QQ群中下载",UIAccess:"启用此选项后，有更高级别的热键、置顶效果",chaojijp:"显示四字及以上的简拼候选",bmhg:"按Ctrl + A-Z快速定位需要修改的编码位置",fyfz:"翻页后进入间接辅助码模式",启用TSF:"安装TSF，切换输入法激活Yzime`n，上屏更快，能在更多界面上屏"
			,EnChar:"勾选后可在开启其他输入法中文输入状态时，`n正常上屏英文字符、标点，但无法响应相关热键",SendDelay:"部分控件无法响应太快的键击导致上屏漏字，`n适当调高延时解决相关问题",Imagine:"拼音4字后超前联想后续字词`n形码类逐码提示",MemoryDB:"将数据库载入内存获取更好的性能，关闭此选项、退出影子时保存数据库",ClipHistory:"记录剪贴板文本历史，clip/显示历史纪录",fixedword:"单字字频不变"
